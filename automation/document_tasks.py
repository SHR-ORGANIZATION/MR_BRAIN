...
def generate_document(path, topic, content=None, doc_type=None):
    from automation.document_generator import generate_assignment, generate_research_proposal, generate_report, save_document
    
    topic_text = topic or Path(path).stem
    
    if doc_type == "assignment":
        doc = generate_assignment(topic_text)
        filename = f"{_sanitize_filename(topic_text)}_Assignment.docx"
        result = save_document(doc, filename, Path(path).parent if "\\" in path or "/" in path else None)
    ...
    if doc_type == "research_proposal":
        ...
        filename = f"{_sanitize_filename(topic_text)}_Proposal.docx"
        result = save_document(doc, filename, Path(path).parent if "\\" in path or "/" in path else None)
        if result.get("status") == "success":
            return _build_result("Created Document", result.get("path", path), "success", result.get("message", ""), "Word Document", result)
        return _build_result("Create Document", path, "failed", result.get("message", ""), "Word Document")
    ...
    if doc_type == "report":
        ...
        filename = f"{_sanitize_filename(topic_text)}_Report.docx"
        result = save_document(doc, filename, Path(path).parent if "\\" in path or "/" in path else None)
        if result.get("status") == "success":
            return _build_result("Created Document", result.get("path", path), "success", result.get("message", ""), "Word Document", result)
        return _build_result("Create Document", path, "failed", result.get("message", ""), "Word Document")
    
    content = content or f"Draft document on {topic_text}.\n\nThis document was generated by the NOVA assistant.\n"
